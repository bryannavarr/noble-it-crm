...
export const createTickets = async (
  payload: CreateTicketPayload | CreateTicketPayload[]
) => {
...
export const listTickets = async (filters: {
  ...
  const [rows] = await pool.execute(
    `SELECT t.*,
            ...
            c.invoice_prefix,
            COALESCE(SUM(wl.qty), 0) AS total_hours_logged
     FROM tickets t
...
export const updateWorkLog = async (
  ...
  if (updates.qty         !== undefined) { fields.push('qty = ?');         params.push(updates.qty); }
  if (updates.unit_price  !== undefined) { fields.push('unit_price = ?');  params.push(updates.unit_price); }
  if (updates.description !== undefined) { fields.push('description = ?'); params.push(updates.description); }
  if (updates.worked_date !== undefined) { fields.push('worked_date = ?'); params.push(updates.worked_date); }

  if (!fields.length) throw new Error('No fields to update');

  await pool.execute(
    `UPDATE work_logs SET ${fields.join(', ')} WHERE id = ? AND ticket_id = ?`,
    [...params, logId, ticketId]
  );
...
export const deleteWorkLog = async (ticketId: number, logId: number) => {
  await pool.execute(
    'DELETE FROM work_logs WHERE id = ? AND ticket_id = ?',
    [logId, ticketId]
  );
};
...
export const updateComment = async (
  ...
  await pool.execute(
    'UPDATE comments SET body = ? WHERE id = ? AND ticket_id = ?',
    [body, commentId, ticketId]
  );
