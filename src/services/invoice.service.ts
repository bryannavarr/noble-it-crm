...
export const generateInvoice = async (clientId: number, month: string) => {
    ...
    lineItems.map(item =>
      pool.execute(
        `INSERT INTO invoice_line_items
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [invoiceId, item.type, item.reference_id, item.category, item.subject, item.qty, item.rate ?? 0, item.amount]
      )
    )
  );
  ...
  await pool.execute(
    `UPDATE invoices SET pdf_path = ?, status = 'PENDING_APPROVAL' WHERE id = ?`,
    [pdfPath, invoiceId]
  );
...
const buildLineItems = async (clientId: number, month: string) => {
  ...
  for (const ticket of tickets) {
    const label   = CATEGORY_LABELS[ticket.category as keyof typeof CATEGORY_LABELS] ?? ticket.category;
    ...
    if (ticket.category === 'HARDWARE') {
      ...
      const totalCost   = logs.reduce((sum: number, l: any) => sum + Number(l.qty) * Number(l.unit_price), 0);
  ...
  for (const meeting of meetings) {
    ...
    const meetingDateStr = String(meeting.meeting_date).substring(0, 10);
  ...
  return new Promise((resolve, reject) => {
    ...
    lineItems.forEach(item => {
      ...
      doc.fillColor(BLACK).fontSize(9).font('Helvetica-Bold')
        .text(item.category, colLabelX, y);
      y += 13;
      ...
      doc.fillColor(BLACK).fontSize(9)
        .text(`$${item.amount.toFixed(2)}`, colAmountX, rowTop, { width: colAmountW, align: 'right' });
      // Hardware shows plain qty, services show qty with 'h' suffix
      const qtyLabel = item.is_hardware
        ? String(item.qty)
        : `${item.qty}h`;
      doc.text(qtyLabel, colHoursX, rowTop, { width: colHoursW, align: 'right' });

      doc.moveTo(margin, y + 4).lineTo(margin + contentW, y + 4)
    ...
    doc.fillColor(BLACK).font('Helvetica-Bold')
      .text(`$${meta.totalAmount.toFixed(2)}`, colAmountX, y, { width: colAmountW, align: 'right' });

    doc.moveTo(totLabelX, y + 16).lineTo(pageW - margin, y + 16)
    ...
    if (billingEmail) {
      doc.fillColor(GRAY).fontSize(8).font('Helvetica')
        .text(
          `For billing inquiries, please contact ${billingEmail}`,
          margin,
          footerY + 20,
